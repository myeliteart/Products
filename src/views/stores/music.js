...
export const useMusicStore = defineStore('music', () => {
  ...
  const selectedFilterCat = ref('beauty')
  ...
  const filteredMusic = computed(() => {
    return products.value.sort((a, b) => {
    ...
    }).filter(item => selectedFilterCat.value == '' || item.category == selectedFilterCat.value &&
      ...
      (!minPrice.value || item.price >= Number(minPrice.value)) && (!maxPrice.value || item.price <= Number(maxPrice.value)) &&
       (checks.value.length === 0 || checks.value.includes(item.category))
    )
  })
