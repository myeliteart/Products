    ...
    <div class="flex flex-col md:flex-row justify-between gap-10" v-if="currentAlbumItem">
        ...
        <div class="my-1 flex-1">
            ...
            <div class="mt-18 mb-4 flex cursor-pointer gap-12">
                <p v-for="tab in tabs" class="stly" @click="matchTab(tab)" :class="selectedTab == tab.toLowerCase().split(' ').join('-').replace('information', 'info') ? 'tabStyle' : ''">{{ tab }} &nbsp; <b>{{ tab.toLowerCase().split(' ').join('-').replace('information', 'info') == 'reviews' ? currentAlbumItem.reviews.length : '' }}</b></p>
            </div>
